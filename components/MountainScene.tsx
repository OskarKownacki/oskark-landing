...
export function MountainScene() {
  ...
  useEffect(() => {
    const layers: Array<[React.RefObject<HTMLElement | SVGElement | null>, number]> = [
  ...
  return (
    ...
    <div aria-hidden style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden", background: "var(--sky-wash)" }}>
      ...
      <div ref={clouds} style={{ position: "absolute", inset: 0, overflow: "hidden", willChange: "transform" }}>
        {[
          { top: "44%", width: "34%", height: 52, blur: 30, opacity: 0.7, dur: "92s", delay: "0s" },
          { top: "38%", width: "46%", height: 40, blur: 30, opacity: 0.85, dur: "140s", delay: "-40s" },
          { top: "56%", width: "26%", height: 34, blur: 30, opacity: 0.75, dur: "118s", delay: "-80s" },
          { top: "64%", width: "38%", height: 30, blur: 30, opacity: 0.60, dur: "168s", delay: "-110s" },
        ].map((c, i) => (
      ...
      {/* ścieżki zamykają się grubo poniżej viewBoxa + overflow:visible — inaczej parallax
          (ujemne k) wysuwa płaską krawędź SVG w kadr jako ostrą poziomą linię */}
      <svg ref={far} viewBox="0 0 1440 540" preserveAspectRatio="none" style={{ position: "absolute", left: "-2%", width: "104%", bottom: 0, height: "48vh", opacity: 0.5, filter: "blur(1px)", overflow: "visible", willChange: "transform" }}>
        <path fill="var(--ridge-far)" d={RIDGE_FAR} />
      </svg>
      <svg ref={mid} viewBox="0 0 1440 540" preserveAspectRatio="none" style={{ position: "absolute", left: "-2%", width: "104%", bottom: 0, height: "38vh", opacity: 0.9, overflow: "visible", willChange: "transform" }}>
        <path fill="var(--ridge-mid)" d={RIDGE_MID} />
      </svg>
      <svg ref={near} viewBox="0 0 1440 540" preserveAspectRatio="none" style={{ position: "absolute", left: "-2%", width: "104%", bottom: 0, height: "44vh", overflow: "visible", willChange: "transform" }}>
        <path fill="var(--ridge-near)" d={RIDGE_NEAR} />
      </svg>
    </div>

    {/* poza sceną (ta jest aria-hidden + pointer-events:none), żeby dysk dało się
        kliknąć i złapać tabem. Klik przełącza motyw: księżyc zachodzi w prawo,
        słońce wschodzi z lewej */}
    <button
      ref={disc}
      onClick={toggle}
      aria-label={theme === "dark" ? "Włącz motyw dzienny" : "Włącz motyw nocny"}
      style={{
        position: "fixed", right: "5%", top: "5%", width: 96, height: 96, zIndex: 0,
        padding: 0, border: 0, background: "none", borderRadius: "50%", cursor: "pointer",
